...
RUN apt-get update && apt-get install -y --no-install-recommends xz-utils file && \
  curl -Ls https://github.com/upx/upx/releases/download/v5.1.1/upx-5.1.1-${TARGETARCH}_linux.tar.xz -o - | tar xvJf - -C /tmp && \
  cp /tmp/upx-5.1.1-${TARGETARCH}_linux/upx /usr/local/bin/ && \
